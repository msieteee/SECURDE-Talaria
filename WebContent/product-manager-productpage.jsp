<%-- 
    Document   : product-manager-productpage
    Created on : 08 10, 16, 8:05:13 PM
			...
			<div class = "row">
				...
				<div class = "col-lg-6">
					<div class = "product-details">					
						<form id="EditProduct" action="EditProduct" method="POST" name="EditProduct">
							<input type="hidden" name="csrfPreventionSalt" value="${csrfPreventionSalt}"/>
                            <c:set var="product" scope="request" value="${product}"/>
							<div class = "row">
								<h6 class="text-space"><b>Name</b></h6>
								<input type="text" name="shipadd" class="text-fld col-lg-10" value="${product.name}">
							</div>
							<div class = "row" name="category" id = "category">
								<h6 class="text-space"><b>Category</b></h6>
								<select class="cust-select col-lg-10" id="category" name="category">	         
	                                <option value="${selectedcat}" selected>${selectedcat}</option>
	                                        <c:forEach items="${categories}" var="categories">
	                                            <c:if test="${categories != selectedcat}">
							...
							<div class = "row">
								<h6 class="text-space"><b>Description</b></h6>
								<textarea value="${product.description}" class="cust-area col-lg-10" rows="5" maxlength="250" name="description">${product.description}</textarea>
							</div>
							<div class = "row">
								<h6 class="text-space"><b>Unit Price</b></h6>
								<input type="number" name="price" class="text-fld col-lg-10" value="${product.price}">
							</div>
							<div class = "row">
							<h6 class="text-space"><b>Quantity</b></h6>
								<input type="number" name="qty" class="text-fld col-lg-2" placeholder="Qty." value="${product.quantity}">
								<div class="col-lg-3 col-lg-offset-1">
                                    <input type="hidden" name="index" id="product-index" value="<%= session.getAttribute("index") %>" />
									<input type="submit" name="login" class="btn btn-lg transact-btn" value="UPDATE">
								</div>
							</div>
